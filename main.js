...
var communities = [
  {
    ...
    schedule: [
      ...
      { day: "Segunda-feira", items: ["4a Segunda do mes, 19h30: Escola da Fe"] },
      { day: "Quarta-feira", items: ["7h30: Missa com Novena de Nossa Senhora do Perpetuo Socorro"] },
      { day: "Quinta-feira", items: ["14hs as 17hs: Brecho no Salao Paroquial"] },
      { day: "Sexta-feira", items: ["1a Sexta do mes, 7h30: Missa do Sagrado Coracao de Jesus, com Adoracao e Bencao do Santissimo Sacramento", "19hs: Terco dos Homens"] },
      { day: "Sabado", items: ["9hs as 12: Brecho no Salao Paroquial", "9hs: Aula de Violao", "10hs: Aula de Tecnica Vocal", "18h30: Missa"] }
    ],
    extra: "Missa com Novena de Santa Luzia: Dia 13 de cada mes - Quando as segundas-feiras, as 19h30. Quando as tercas, quartas, quintas ou sextas-feiras, as 7h30."
  },
  {
    ...
    schedule: [
      ...
      { day: "Quinta-feira", items: ["15hs: Terco da Misericordia e Terco Mariano"] },
  ...
  },
  {
    ...
    schedule: [
      ...
      { day: "Sexta-feira", items: ["1a Sexta do mes, 19h30: Missa do Sagrado Coracao de Jesus, com Adoracao e Bencao do Santissimo Sacramento"] },
  ...
  },
  {
    ...
    schedule: [
      ...
      { day: "Sexta-feira", items: ["1a Sexta do mes, 19hs: Devocao do Sagrado Coracao de Jesus"] },
      { day: "Domingo", items: ["08h30: Missa/Celebracao da Palavra"] }
    ]
  },
  {
  ...
  },
  {
    ...
    schedule: [
      ...
      { day: "Quarta-feira", items: ["15hs: Adoracao ao Santissimo Sacramento", "4a Quarta de mes, 19:30hs: Missa"] },
  ...
  },
  {
    ...
    schedule: [
      ...
      { day: "Terca-feira", items: ["14h: Artesanato no Salao Social", "19h30: Momento de Oracao, Forca e Fe"] },
      { day: "Sabado", items: ["16h30: Santo Terco", "17hs: Missa/Celebracao da Palavra"] }
    ]
  },
  {
    ...
    schedule: [
      { day: "Terca-feira", items: ["14h30 as 17h30: Clubinho do Artesanato no Salao Social"] },
      { day: "Quarta-feira", items: ["3a Quarta do mes, 19h30: Missa"] },
      { day: "Quinta-feira", items: ["15hs: Adoracao ao Santissimo Sacramento"] },
      { day: "Domingo", items: ["8h30: Missa/Celebracao da Palavra"] }
    ]
  }
];
...
function buildCard(c) {
  ...
  var extraHTML = c.extra ? '<div class="card-extra">' + c.extra + '</div>' : '';
  var badgeHTML = c.featured ? '<span class="card-badge">Sede da Paroquia</span>' : '';
  ...
  card.innerHTML =
    ...
    '<p class="card-address"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>' + c.address + '</p>' +
    '</div>' +
    '</div>' +
    badgeHTML +
    '</div>' +
  ...
  card.addEventListener('click', function () {
    ...
    document.querySelectorAll('.community-card.expanded').forEach(function (c2) {
      c2.classList.remove('expanded');
      c2.querySelector('.card-footer').textContent = 'Clique para ver todos os horarios';
    });
    if (!isExpanded) {
...
function initScrollReveal() {
  ...
  document.querySelectorAll('.reveal, .reveal-left, .reveal-right, .reveal-scale, .reveal-rotate').forEach(function (el) {
    observer.observe(el);
  });
}
...
function copyPix(text, btn) {
  navigator.clipboard.writeText(text).then(function () {
    ...
    setTimeout(function () {
      ...
      btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg> Copiar';
    }, 2000);
  });
}
...
async function loadLiturgia() {
  ...
  try {
    ...
    try {
      ...
      var responseBackup = await fetch('https://api-liturgia-diaria.vercel.app/?date=' + hoje, {
        method: 'GET'
      });
      ...
      if (salmoRefraoEl) {
        salmoRefraoEl.querySelector('.refrao-text').textContent = 'Indisponivel';
      }
    }
  }
  ...
  function preencherLiturgiaPrincipal(data) {
    if (liturgiaDateEl) {
      ...
      liturgiaDateEl.innerHTML =
        ...
        liturgiaStr +
        ' <span style="color: ' + getCorLiturgica(corStr) + ';">(' + corStr + ')</span>';
    }
    ...
    if (data.leituras && data.leituras.evangelho && data.leituras.evangelho.length > 0) {
      ...
      if (evangelhoTextEl) {
        evangelhoTextEl.innerHTML = '<p>' + formatLiturgiaText(evangelho.texto || 'Texto nao disponivel.') + '</p>';
      }
    } else {
      if (evangelhoRefEl) evangelhoRefEl.textContent = 'Evangelho do dia';
      if (evangelhoTextEl) evangelhoTextEl.innerHTML = '<p>Evangelho nao encontrado.</p>';
    }

    if (data.leituras && data.leituras.salmo && data.leituras.salmo.length > 0) {
      ...
      if (salmoTextEl) {
        salmoTextEl.innerHTML = formatSalmoText(salmo.texto || 'Texto nao disponivel.');
      }

      if (salmoRefraoEl) {
        salmoRefraoEl.querySelector('.refrao-text').textContent = salmo.refrao || 'Refrao nao disponivel.';
      }
    } else {
      if (salmoRefEl) salmoRefEl.textContent = 'Salmo do dia';
      if (salmoTextEl) salmoTextEl.innerHTML = '<p>Salmo nao encontrado.</p>';
      if (salmoRefraoEl) salmoRefraoEl.querySelector('.refrao-text').textContent = 'Refrao nao encontrado.';
    }
  }

  function preencherLiturgiaReserva(data) {
    if (liturgiaDateEl) {
      liturgiaDateEl.innerHTML = '<strong>' + (data.data || 'Hoje') + '</strong>';
    }

    if (evangelhoRefEl) {
      evangelhoRefEl.textContent = data.referencia_evangelho || 'Evangelho do dia';
    }

    if (evangelhoTextEl) {
      evangelhoTextEl.innerHTML =
        '<p>' + formatLiturgiaText(data.evangelho || 'Evangelho nao encontrado.') + '</p>';
    }
    ...
    if (salmoTextEl) {
      salmoTextEl.innerHTML = formatSalmoText(data.salmo || 'Salmo nao encontrado.');
    }
...
function toggleLiturgiaExpand(cardId) {
  ...
  if (btnText) {
    btnText.textContent = card.classList.contains('expanded') ? 'Recolher' : 'Ler mais';
  }
}
...
var eventosData = {
  'visita-reliquia': {
    badge: 'Programação',
    titulo: 'CONFIRA NOSSA PROGRAMAÇÃO',
    imagem: 'assets/juc.jpeg',
    texto: [
      '🔥 Lual Luz de Maria',
      '📅 16 de Maio | 18h30',
      '📍 R. Maestro Salvador Bove, 327',
      'Jd. Nova Europa, Campinas - SP',
      '⛪ Missa dos Jovens',
      '📅 31 de Maio | 10h00',
      '📍 Rua René Fernandes, 71',
      'Jd. dos Oliveiras | Campinas/SP',
      '🎉 Festa Junina | Paróquia Santa Luzia',
      '📅 30, 31/05 • 06, 07, 13 e 14/06',
      '🙏 Precisaremos do seu apoio',
      'Jovens Unidos em Cristo'
    ],
    bencao: 'Jovens Unidos em Cristo',
    destaques: [
      { icone: 'calendario', titulo: 'Programação', valor: '16 de Maio a 14 de Junho' }
    ]
  },
  'semana-santa': {
...
function getIconeSvg(tipo) {
  var icones = {
    'calendario': '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>',
    'relogio': '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
...
function openEventoModalDinamico(eventoId) {
  ...
  var textoParagrafos = evento.texto.map(function (p) {
    return '<p>' + p + '</p>';
  }).join('');

  var destaquesHtml = evento.destaques.map(function (d) {
    return '<div class="evento-modal-destaque">' +
      '<div class="destaque-icon">' + getIconeSvg(d.icone) + '</div>' +
      '<div><strong>' + d.titulo + '</strong><span>' + d.valor + '</span></div>' +
      '</div>';
  }).join('');
  ...
  modalBody.innerHTML =
    ...
    '<div class="evento-modal-badge">' + evento.badge + '</div>' +
    '<h3 class="evento-modal-titulo">' + evento.titulo + '</h3>' +
    '<div class="evento-modal-texto">' + textoParagrafos +
    '<p class="evento-modal-bencao">' + evento.bencao + '</p>' +
    '</div>' +
    '<div class="evento-modal-destaques">' + destaquesHtml + '</div>' +
    '</div>';
